...
with lib; {
  options.modules.desktop.gaming.emulators = let
    mkBoolDefault = bool:
      mkOption {
        type = types.bool;
        default = bool;
  ...
  config = let emu = config.modules.desktop.gaming.emulators;
  in {
    my.packages = with pkgs;
      (if emu.psx.enable then [ epsxe ] else [ ])
      ++ (if emu.ds.enable then [ desume ] else [ ])
      ++ (if (emu.gba.enable || emu.gb.enable || emu.snes.enable) then
        [ higan ]
      else
        [ ]);
  };
